...
class VideoToTextDecoder:
    ...
    def decode(self, video_path):
        ...
        try:
            ...
            # Scansione iniziale per metadati (primi 5 frame)
            for _ in range(5):
                ret, frame = cap.read()
                if not ret:
                    break
            ...
            while True:
                ...
                for obj in decoded_objects:
                    try:
                        ...
                        # Metadati (ridondante, ma per robustezza)
                ...
                # Break se abbiamo tutti i chunk
                if expected_chunks and len(chunks) >= expected_chunks:
                    break
            
            cap.release()
            
            # Verifica chunk mancanti
            if expected_chunks and len(chunks) < expected_chunks:
